...
static PyObject *_pymcce_setup_system(PyObject *self, PyObject *args)
{
   ...
   if (env.do_premcce) {
      ...
      if (premcce()) {db_close(); return Py_BuildValue("i", USERERR);}
...
static PyObject *_pymcce_generate_rotamers(PyObject *self, PyObject *args)
{
   ...
   if (env.do_rotamers) {
      ...
      if (rotamers()) {
      db_close(); return Py_BuildValue("i", USERERR);
      }
...
static PyObject *_pymcce_calculate_energy(PyObject *self, PyObject *args)
{
   ...
   if (env.do_energies) {
      ...
      if (energies()) {db_close(); return Py_BuildValue("i", USERERR);}
...
static PyObject *_pymcce_generate_samples(PyObject *self, PyObject *args)
{
   ...
   if (env.do_monte) {
      ...
      if (monte()) {db_close(); return Py_BuildValue("i", USERERR);}
           else printf("Step 4 Done.\n\n");
       }
       else {
           if (monte2()) {db_close(); return Py_BuildValue("i", USERERR);}
           else printf("Step 4 Done.\n\n");
       }
   }
...
static PyMethodDef _pymcceMethods[] = {
   ...
   {"initialize", 
   (PyCFunction)_pymcce_initialize, 
   METH_VARARGS,
   "Initialize MCCE calculations"
   },
